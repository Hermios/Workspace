...
class GraphDatabase{
	...
	static <NL extends Enum<NL>> Node createNode(NL labelName)
	{
		Node result=null;
	...
	static <NL extends Enum<NL>> Node createNode(NL labelName,INodePropertyKey key,Object value,boolean getExistingOne)
	{
		Node result=null;
		Label label=DynamicLabel.label(labelName.toString());		
		try(Transaction tx=getInstance()._graph.beginTx())
	...
	static <NL extends Enum<NL>> List<Node> findNodesByLabelAndProperty(NL labelName,INodePropertyKey key,Object value)
	{
	...
	static <NL extends Enum<NL>> List<Node> findNodesByLabelAndRegexProperty(NL labelName,String regexValue,INodePropertyKey... keys)
	{
		List<Node> result=new ArrayList<>();
	...
	static <NL extends Enum<NL>> List<Node> getNodes(NL nodeLabel)
	{
		List<Node> result=new ArrayList<>();	
		try(Transaction tx=getInstance()._graph.beginTx())
		{
